...
public class OrderFacadeImpl implements OrderFacade {
    ...
    public OrderDto prepareOrderForPayment(UUID orderId) {
        ...
        orderContext = enrichWeightVolumeFragile(orderContext);
        orderContext = enrichFromAddress(orderContext);
        orderContext = registerDelivery(orderContext);
        orderContext = calculateDeliveryPrice(orderContext);
        orderContext = calculateProductPrice(orderContext);
        orderContext = calculateTotalPrice(orderContext);
        orderContext = registerPayment(orderContext);
    ...
    private OrderContext registerPayment(OrderContext orderContext) {
        ...
        if (orderContext.getPaymentId() == null) {
            ...
            orderContext = orderContext.toBuilder()
                    ...
                    .build();
            log.debug("Facade. Запрос по регистрации платежа для заказа с id {} обработан, доставке присвоен id {}.",
