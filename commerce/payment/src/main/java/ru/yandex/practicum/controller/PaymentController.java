...
public class PaymentController implements PaymentClient {
    ...
    @PostMapping
    public PaymentDto handlePayment(@RequestBody OrderDto request) {
        log.info("Получен POST-запрос на оплату заказа с id {}.",
                request.getOrderId());
        return paymentFacade.handlePayment(request);
    }

    @PostMapping("/totalCost")
    public BigDecimal calculateTotalCost(@RequestBody OrderDto request) {
        log.info("Получен POST-запрос на расчет полной стоимости заказа с id {}.",
                request.getOrderId());
        return paymentFacade.calculateTotalCost(request);
    }

    @PostMapping("/productCost")
    public BigDecimal calculateProductCost(@RequestBody OrderDto request) {
        log.info("Получен POST-запрос на расчет стоимости товаров в заказе с id {}.",
                request.getOrderId());
        return paymentFacade.calculateProductCost(request);
    }
