...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PaymentDto createPayment(OrderDtoPayment request) {
        ...
        BigDecimal feeTotal = calculateFeeTotal(productPrice);
        ...
        log.debug("Service. Запроса на регистрацию оплаты заказа с id {} обработан, .", request.getOrderId());
    ...
    public BigDecimal calculateProductPrice(List<ProductPrice> itemsWithPrice) {
        ...
        BigDecimal productPrice = itemsWithPrice.stream().map(item ->
    ...
    private BigDecimal calculateFeeTotal(BigDecimal productPrice) {
        return productPrice.multiply(TAX_RATE).setScale(2, RoundingMode.HALF_UP);
    }
